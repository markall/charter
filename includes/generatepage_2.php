...
// *****************************************************************************
 ...
 //**********************************************************************
        ...
        if (strpos($s,'<%contactsasoptions%>')>0) {
                 $t =     contactlist($contact->userid,'');
                 $s = str_replace("<%contactsasoptions%>", $t  , $s);       //all contacts
        }
        ...
        if ( strpos($s,'<%contactsbycategory%>' ) ) {
          ...
          if (strpos($s,'<%contactsbycategory=')>0) {
                 ...
                 while ($i>0) {
                         ...
                         $offset=0;
                         $perpage=99999;
                         $t = contactsbycategory($gcategory->id,"active",'', $directoryitemtemplate,"lastname",$searchstr,$sel );
...
//            echo $vatrate."<br/>";
            ...
            $s = str_replace('<%product_mainimg%>',$sitedomain."/shop/".$category->media[count($category->media)-1]->mediaf,$s);
...
//                  $s=$s."<p><i><small>"."<%childcontactcompany%> "."<%childcontacttelephone1%>"." "."<%childcontactusername%>"." "."<%contactpassword%>"."</small></i></p>";
                        ...
                        $contacti->selected = "";
                        if ($id==$contacti->userid) {
                                $contacti->selected = "selected";
                        }

                        $s=tidytags(setcontact($s,$contacti,$contactdi, '' ));
...
//                     $s = str_replace('<%".$prefix."contact_media['.$idx.']%>',$value['mediaf'], $s);
        ...
        function contactlist($id,$type) {
            include_once INCLUDES_FOLDER."db_contact.php";
            $crypt = new crypt();
            $o='';
            $recs=contact::getallcontacts( '',$type,0,2000,' ORDER BY company, status desc,sortorder ' );
